...
func defaultAgentConfigPath() string {
	if runtime.GOOS == "windows" {
		return filepath.Join(os.Getenv("ProgramData"), "nossh", "agent.yaml")
	}
	return "/etc/nossh/agent.yaml"
}
...
func main() {
	...
	configPath := flag.String("config", defaultAgentConfigPath(), "agent config path")
	flag.Parse()
